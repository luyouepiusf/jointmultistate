##' Calculate the information matrix (based on the observed likelihood)
##'
##' @title Calculate the information matrix (based on the observed likelihood)
...
info_observed<-function(model_fit,nqmc=2000){
  ...
  result_info_observed<-info_observed_covariate(
  ...
  possible_transitions_begin<-c(result_info_observed[[5]])+1
  possible_transitions_end<-c(result_info_observed[[6]])+1
  
  II_beta_beta_expand<-field_to_matrix(result_info_observed[[1]])
  II_beta_hazard_expand<-field_to_matrix(result_info_observed[[2]])
  F_hazard_hazard_expand<-result_info_observed[[3]]
  F_hazard0_count<-lapply(result_info_observed[[4]],c)
  
  list_info_observed<-vector("list",nstates)
  ...
  for(idx_begin in 1:nstates){
    if(any(possible_transitions[idx_begin,])){
      ...
      idx_row<-II_beta_beta_expand$idx_row%in%all_idx_transitions
      idx_col<-II_beta_beta_expand$idx_col%in%all_idx_transitions
      idx_hazard<-F_hazard0_count[[idx_begin]]>0
      
      II_beta_beta_mat<--II_beta_beta_expand$mat[idx_row,idx_col]
      II_beta_hazard_mat<--II_beta_hazard_expand$mat[idx_row,idx_hazard]
      II_hazard_hazard_mat<--F_hazard_hazard_expand[[idx_begin]][idx_hazard,idx_hazard]
      
      info_mat0<-
        II_beta_beta_mat[-1,-1]-
          II_beta_hazard_mat[-1,]%*%
          MASS::ginv(II_hazard_hazard_mat)%*%
          t(II_beta_hazard_mat[-1,])
      info_mat<-rbind(0,cbind(0,info_mat0))
      list_info_observed[[idx_begin]]<-info_mat
    }
  }
  return(list_info_observed)
}
